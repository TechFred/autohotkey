...
left_menu_region := [0, 0, 1950, 1024]
splash_region := [0, 0, 1950, 1024]


; Activate the game window
IfWinExist, %winTitle%

; Maximize the window adn activate
...
FindImage("C:\Users\Fred\Git\autohotkey\Assets\Darkwars_hospital\optimise_food.bmp", AllRegion, 100, 1000, true)
...
;Find image in a region and click it
FindImage(Path, Region, tolerance := 0, sleepDelay := 1000, doClick := true) {
    x1 := Region[1]
    y1 := Region[2]
    x2 := Region[3]
    ...
    ImageSearch, xH, yH, x1, y1, x2, y2, *%tolerance% %Path%
    
    result := { x: xH, y: yH, ErrorLevel: ErrorLevel }

    if (ErrorLevel = 0 && doClick) {
        MouseClick, left, xH, yH
        Sleep, sleepDelay
    }
